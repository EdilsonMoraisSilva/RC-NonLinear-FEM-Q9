...
%
% Example II -- Aurich & Campos Filho beam (2003)
%   Simply supported RC beam, four-point bending (two symmetric loads).
%   Geometry : L = 2.50 m, h = 0.25 m, b = 0.12 m
%   Loading  : total P = 50 kN (P/2 at x = 0.783 m and x = 1.717 m),
%              applied in 20 monotonic load steps
%
% Units: kN, m, MPa
%==========================================================================

function Input_Example_II
...
fprintf('=Example II -- Aurich & Campos Filho beam (2003), total load 50 kN       =\n');
...
% built in five horizontal strips so that the bottom and top
% reinforcement layers can be aligned with element boundaries.

    % Number of DOFs per node (in-plane membrane: ux, uy)
    Dados.NumGDLNo = 2;

    % Slab edge lengths
    Lx = 2.50;     % beam length      (m)
    Ly = 0.25;     % beam depth       (m)
    ...
    % Segment table:
    %               Lx       Ly        Nx   Ny
    Segment =  [   Lx   0.01500    nx    1     % bottom cover
                   Lx   0.01000    nx    1     % bottom reinforcement layer
                   Lx   0.20185    nx    6     % web
                   Lx   0.00630    nx    1     % top reinforcement layer
                   Lx   0.01685    nx    1 ];  % top cover
...
% external load is applied. The reinforcement layers are aligned with
% the loading positions by adjusting selected nodal x-coordinates.
    ...
    for i = 1:length(Dados.conect)
        ...
        Dados.ele(i, :)       = [3820e4  0.2  0.12   3       0   0   0];
        Dados.TipoElemento(i, :) = 'ElemMembrana';
    end

    % --- Applied load (four-point bending) ------------------------------
    Dados.CargaNo = zeros(Dados.NumNos, Dados.NumGDLNo);
    Dados.P       = -25;                % kN per loading point (P/2 each)

    % Loading positions: third points of the span, on the top face
    PosCarga = [ 0.783   0.25
                 1.717   0.25 ];

    % ---------------------------------------------------------------------
    % Adjust nodal x-coordinates so that the loading-point columns of
    % the mesh fall exactly at x = 0.783 m and x = 1.717 m. Without this
    % alignment the search loop below would fail to find the load nodes.
    % ---------------------------------------------------------------------
    for m = 0:(2*ny)
        % Local node indices (within the row) of the two columns to move
        P(1) = 7;
        P(2) = 15;

        % Direction of motion: 1 = x, 2 = y
        d = 1;

        no(1) = P(1) + m*(2*nx + 1);
        no(2) = P(2) + m*(2*nx + 1);

        Dados.coords(no(1), d) = 0.783;
        Dados.coords(no(2), 1) = 1.717;
    end

    % Locate the two loading nodes after coordinate adjustment
    cont = 1;
    for i = 1:Dados.NumNos
        if (PosCarga(cont,1) == Dados.coords(i,1)) && ...
           (PosCarga(cont,2) == Dados.coords(i,2))
            noCarga(cont) = i;
            cont = cont + 1;
        end
        if cont > size(PosCarga, 1)
            break
        end
    end
    ...
    % Bottom face, mid-span (between the two loading points).
    PosDeslNo = [1.25 0];
    for i = 1:Dados.NumNos
...
% by Obtemtaxaarmadura.
    ...
    switch Dados.TipoArm
        case 'Distributed'
            ...
            % Bottom layer : 2 phi 10 mm (longitudinal tension reinforcement)
            % Top layer    : 2 phi 6.3 mm (longitudinal compression reinforcement)
            %
            %                       n.bars   diameter   L     x1   x2    y1      y2      z1   z2
            Dados.Arm.Barras = { [    2       0.01000   Lx   0.0  Lx   0.0160  0.0240  0.0  0.0
                                      2       0.00630   Lx   0.0  Lx   0.2270  0.2331  0.0  0.0 ]
...
% Uniaxial monotonic envelope parameters used by ParametrosAco.
    ...
    Dados.Arm.fpy = 54.9e4;     % Steel yield stress       (kPa)
    Dados.Arm.Ei  = 21000e4;    % Initial elastic modulus  (kPa)
    Dados.Arm.fpu = 65.9e4;     % Ultimate stress          (kPa)
    Dados.Arm.Ey  = 292.6e4;    % Hardening tangent modulus (kPa)
